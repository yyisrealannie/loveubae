-- 每篇动态的首次评论在发布后 12 小时内出现；后续往返回复保持 2–9 分钟。
  ...
  for cfg in select * from public.milk_moments_config where enabled and jsonb_array_length(cards) > 0 limit 100 loop
    ...
    loop
      if p.partner_comment_due_at is null then
        update public.milk_moments_posts
          set partner_comment_due_at = p.created_at + (1 + random() * 718) * interval '1 minute'
    ...
    loop
      if c.due_at is null then
        update public.milk_moments_comments set due_at = tick_now + (2 + random() * 7) * interval '1 minute'
